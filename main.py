...
for sector in fig2['sector'].unique():
    ...
    for flow_long in fig2['flow_long'].unique():
        ...
        fig.add_trace(go.Scatter(
                name=str(flow_long).replace('Domestic navigation','Shipping').replace('Domestic aviation','Aviation').replace('Transport not elsewhere specified','Other'),
                line=dict(width=1, color=colors[random.randrange(20)]),
                x=fig2[(fig2["sector"] == sector) & (fig2["flow_long"] == flow_long)]["year"],
                y=fig2[(fig2["sector"] == sector) & (fig2["flow_long"] == flow_long)]["% Adoption"],
                fill="tonexty",
                stackgroup="two",
        ...
        fig.add_trace(
            ...
            name="Historical",
            line=dict(width=1, color="#1c352d"),
            x=fig2[(fig2["year"] <= data_end_year) & (fig2["sector"] == sector) & (fig2["flow_long"] == flow_long)]["year"],
            y=fig2[(fig2["year"] <= data_end_year) & (fig2["sector"] == sector) & (fig2["flow_long"] == flow_long)]["% Adoption"],
            stackgroup="one",
...
# Percent of buildings energy that is electric or nonelectric renewables
buildings =  energy_output.loc[model, scenario, region, ['Commercial','Residential'], product_category, slice(None), ['ELECTR', 'SOLARTH', 'MUNWASTER','GEOTHERM'], flow_category, slice(None),['RESIDENT','COMMPUB']].loc[:, start_year:end_year].groupby(groupby).sum().divide(energy_output.loc[model, scenario, region, ['Commercial','Residential'], product_category, slice(None), slice(None), slice(None), slice(None)].loc[:, start_year:end_year].groupby(groupby).sum().sum(0)) * 100
...
agriculture = afolu_output.loc[model, scenario, region, sector, product_long, flow_category, flow_long, unit]
...
electricity = (energy_output.loc[model, scenario, region, ['Electric Power'], product_category, slice(None), ["GEOTHERM", "HYDRO", "SOLARPV","ROOFTOP", "SOLARTH", "OFFSHORE", "ONSHORE", "TIDE","NUCLEAR"], ['Electricity output'], slice(None)].loc[:, start_year:end_year].groupby(groupby).sum()).reindex(axis='index', level=2, labels=['Nuclear','Hydro','Onshore wind energy','Offshore wind energy','Utility solar photovoltaics','Rooftop solar photovoltaics','Solar thermal','Tide, wave and ocean','Geothermal']) * 0.0002778
...
# Percent of buildings energy that is electric or nonelectric renewables
buildings =  energy_output.loc[model, scenario, region, ['Commercial','Residential'], product_category, slice(None), ['ELECTR', 'SOLARTH', 'MUNWASTER','GEOTHERM'], flow_category, slice(None),['RESIDENT','COMMPUB']].loc[:, start_year:end_year].groupby(groupby).sum()
...
# Percent of buildings energy that is electric or nonelectric renewables
buildings =  energy_output.loc[model, scenario, region, ['Commercial','Residential'], product_category, slice(None), ['ELECTR', 'SOLARTH', 'MUNWASTER','GEOTHERM'], flow_category, slice(None),['RESIDENT','COMMPUB']].loc[:, start_year:end_year].groupby(['sector','product_category','flow_long']).sum().divide(energy_output.loc[model, scenario, region, ['Commercial','Residential'], product_category, slice(None), slice(None), slice(None), slice(None)].loc[:, start_year:end_year].groupby(['sector','product_category','flow_long']).sum().sum(0)) * 100
    ...
    for flow in fig2['flow_long'].unique():
        ...
        fig.add_trace(
            go.Scatter(
                ...
                line=dict(width=1, color=colors[
                    pd.DataFrame(fig2['flow_long'].unique()).set_index(0).index.get_loc(flow)]),
                x=fig2[(fig2["sector"] == sector) & (fig2["flow_long"] == flow)]["year"],
                y=fig2[(fig2["sector"] == sector) & (fig2["flow_long"] == flow)]["% Adoption"],
                fill="tonexty",
                stackgroup="two",
        ...
        go.Scatter(
            name="Historical",
            line=dict(width=1, color="#1c352d"),
            x=fig2[(fig2["year"] <= data_end_year) & (fig2["sector"] == sector) &(fig2["flow_long"] == flow)]["year"],
            y=fig2[(fig2["year"] <= data_end_year) & (fig2["sector"] == sector) &(fig2["flow_long"] == flow)]["% Adoption"],
            stackgroup="one",
