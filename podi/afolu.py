...
def afolu(scenario, data_start_year, data_end_year, proj_end_year):
    ...
    for subvertical in (
        afolu_historicalplot[(afolu_historicalplot.reset_index().unit == "Mha").values]
        .reset_index()
        .variable.unique()
    ):
    ...
    # Plot afolu_historical [% of max_extent]
    # region
    afolu_historicalplot = afolu_historical.copy().multiply(100)

    for subvertical in afolu_historicalplot.reset_index().variable.unique():
        afolu_historicalplot[
            afolu_historicalplot.index.get_level_values(3).isin([subvertical])
        ].T.plot(
            legend=False,
    ...
    afolu_analogs.multiply(100).T.plot(
        title="Historical Analog Adoption", ylabel="% of max extent"
    )
    ...
    for scenario in afolu_outputplot.reset_index().scenario.unique():
        for subvertical in afolu_output.reset_index().variable.unique():
            afolu_outputplot[
                (afolu_outputplot.index.get_level_values(3).isin([subvertical]))
                & (afolu_outputplot.index.get_level_values(1).isin([scenario]))
            ].T.plot(
                legend=False,
    ...
    for subvertical in (
        max_extent[(max_extent.reset_index().unit == "Mha").values]
        .reset_index()
        .variable.unique()
    ):
        max_extentplot[
    ...
    for scenario in afolu_outputplot.reset_index().scenario.unique():
        for subvertical in (
            afolu_outputplot[(afolu_outputplot.reset_index().unit == "Mha").values]
            .reset_index()
            .variable.unique()
        ):
            afolu_outputplot[
