...
def energy(scenario, data_start_year, data_end_year, proj_end_year):
    ...
    recalc_energy_reductions = False
    # region
    if recalc_energy_reductions == True:
        ...
        addtl_eff = addtl_eff.groupby(
            ["Region", "Sector", "IEA Product", "Flow"]
        ).mean()
        ...
        energy_post_electrification = pd.DataFrame(
            pd.read_csv("podi/data/energy_" + scenario + ".csv")
        ).set_index(index)
        energy_post_electrification.columns = (
            energy_post_electrification.columns.astype(int)
        )
