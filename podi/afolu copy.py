...
def afolu(scenario, data_start_year, data_end_year, proj_end_year):
    ...
    # Estimate the potential maximum extent of adoption for each AFOLU pathway. Estimates are initially provided as a step function, with 0-2 steps over a period of 80 years. These are translated into a timeseries where the value in each year represents the potential maximum extent of adoption in that year. Values are expressed in MHa land are or Tgdm for biochar.

    # region
    tnc_input_data = pd.read_csv("podi/data/tnc_input_data.csv")

    max_extent = tnc_input_data[tnc_input_data["Metric"] == "Max extent"].drop(
        columns=["iso", "Metric", "Model", "Scenario", "Region", "Unit"]
    )
    ...
    # Estimate values for the average mitigation potential flux (the mass of GHGs that are mitigated per year) for each AFOLU pathway. Estimates are initially provided as a step function, with 0-2 steps over a period of 80 years. These are translated into a timeseries where the value in each year represents the average mitigation potential flux in that year. Values are expressed in tCO2e/ha/yr, tCO2e/m^3/yr for improved forest management, or MtCO2/Tgdm/yr for biochar.

    # region

    flux = tnc_input_data[
        tnc_input_data["Metric"] == "Avg mitigation potential flux"
    ].drop(columns=["iso", "Metric", "Model", "Scenario", "Region", "Unit"])
    ...
    flux2.loc[:, proj_end_year] = flux["Value 1"].values
    ...
    # Load values for historical observations df (as % of max extent) for each AFOLU pathway.

    # region
    tnc_hist_obs = pd.DataFrame(
        pd.read_csv("podi/data/tnc_hist_obs.csv").drop(
            columns=["iso", "Metric", "Model", "Scenario", "Region", "Unit"]
        )
    ).set_index(["Country", "Subvector"])
    tnc_hist_obs.columns = tnc_hist_obs.columns.astype(int)
    tnc_hist_obs = tnc_hist_obs[
        ~tnc_hist_obs.index.get_level_values(1).isin(
            [
    ...
    tnc_hist_obs.interpolate(axis=1, limit_area="inside", inplace=True)
    tnc_hist_obs.update(
        tnc_hist_obs.fillna(0)
        .parallel_apply(
            lambda x: x.divide(
    ...
    # Compute adoption curves of historical analogs used to estimate future potential growth of AFOLU pathways

    # region
    acurves = (
        pd.DataFrame(pd.read_csv("podi/data/tnc_analogs.csv"))
    ...
    avoid.loc[:, data_end_year:] = -avoid.loc[:, data_end_year:].parallel_apply(
        lambda x: x.subtract(
            avoid.loc[x.name[0], x.name[1], x.name[2], :][data_end_year].values[0]
        ),
        axis=1,
    )
    ...
    # shift mitigation values by data_end_year value
